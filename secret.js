...
"[ 0.402 ] Strategic engine DESTRUDO: last signal 6942 A.E.",
...
"",
"[ 0.621 ] Psychospheric cascade event archived.",
"[ 0.650 ] Collateral estimate: 4.1B fatalities.",
"[ 0.689 ] Cause: uncontrolled collective override.",
"",
"",
...
function unlock() {
    ...
    const A = normalizeInput(document.getElementById("hintA").value);
    const B = normalizeInput(document.getElementById("hintB").value);
    const C = normalizeInput(document.getElementById("hintC").value);
    ...
    try {
        const decrypted = CryptoJS.AES.decrypt(encryptedData, hash.toString());
...
const fontSize = 16;
const drops = []; // 各列の現在のY位置
const columnTexts = []; // 各列が担当する文章のインデックス
...
function draw() {
    ...
    for (let i = 0; i < drops.length; i++) {
        const textIdx = columnTexts[i];
        const fullText = texts[textIdx];
        
        // 現在の行に対応する文字を取得（ループさせる）
        const charIdx = Math.floor(drops[i]) % fullText.length;
